...
export function useRoutineData() {
  ...
  const tasksRef = useRef([]); // addTaskが最新のtasksを参照するための同期ミラー(往復通信を増やさないため)
  useEffect(() => {
  ...
  const addTask = useCallback(async ({ name, type, daysOfWeek, date, category }) => {
    ...
    const maxSortOrder = tasksRef.current.reduce((m, t) => Math.max(m, t.sortOrder ?? -1), -1);
    const payload = {
      user_id: user.id,
      name,
      type,
  ...
  // タスク管理モーダルでのドラッグ並び替え確定時に呼ぶ。orderedIdsは新しい表示順のタスクid配列
  // (全件分)。1件ずつupdateすると途中で失敗した場合にDBの並び順が中途半端になりうるため、
  // reorder_routine_tasks RPC(1トランザクションでまとめて更新)を使う。
  const reorderTasks = useCallback(async orderedIds => {
    setStatus("syncing");
    setTasks(prev => {
      const orderIndex = new Map(orderedIds.map((id, i) => [id, i]));
      return prev.map(t => (orderIndex.has(t.id) ? { ...t, sortOrder: orderIndex.get(t.id) } : t));
    });
    const { error } = await supabase.rpc("reorder_routine_tasks", { p_ids: orderedIds });
    if (error) {
  ...
  const updateCategory = useCallback(async (id, { name, colorKey }) => {
    setStatus("syncing");
    setCategories(prev => prev.map(c => (c.id === id ? { ...c, name, colorKey } : c)));
    const { error } = await supabase.from("routine_categories").update({ name, color_key: colorKey }).eq("id", id);
    if (error) {
        ...
        const { data, error } = await supabase
          ...
          .insert(
            importTasks.map((t, i) => ({
              ...
              // エクスポートされたJSON内の並び順(配列の並び)をインポート後も保つ。
              sort_order: i,
            }))
          )
          .select();
